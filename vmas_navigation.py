...
from custom_scenario_utils import CustomScenarioUtils

if typing.TYPE_CHECKING:
    from vmas.simulator.rendering import Geom


class MultiUAVNavigation(BaseScenario):

    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ################
        # Scenario configuration
        ################
        self.plot_grid = True  # You can use this to plot a grid under the rendering for visualization purposes
        ...
        CustomScenarioUtils.check_kwargs_consumed(kwargs) # Warn if not all kwargs have been consumed
        ...
        for i in range(self.n_agents):
            ...
            goal = Landmark(
                ...
                collide=False,
                color=color,
                shape=Box(length = 1, width =  1, hollow = False),
            )
    ...
    def reset_world_at(self, env_index: int = None):
        CustomScenarioUtils.spawn_entities_fixed_obstacles(
            self.world.agents
            + self.goals,  # List of entities to spawn
            self.obstacles,
            self.obstacles_initial_positions, 
            self.world,
    ...
    def extra_render(self, env_index: int = 0) -> "List[Geom]":
        ...
        geoms = [
            CustomScenarioUtils.plot_entity_rotation(agent, env_index)
            for agent in self.world.agents
            if not isinstance(agent.dynamics, Holonomic)
